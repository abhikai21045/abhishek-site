...
Out of curiosity, I tried a few highly predictable and common password patterns. To my surprise, I authenticated into the main administrator account almost immediately. The password was incredibly weak. It was essentially a default setup. I didn't even need Burp Suite Intruder or run a dictionary attack to get in.
...
*   **Never trust the URL:** Never assume a user is allowed to view a page just because they know the web address. The server should actively check the user's permissions on every single request to ensure they actually have requested rights for any given action.